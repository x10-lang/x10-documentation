...
\xcd`(x:Long):Long => x*x` is a function literal describing the squaring function on
integers.  Every function type also posses the (default) value \xcd`null`.

%% CHECK vj: What is ``or constraints'' supposed to mean?
...
\xcd`x1` and \xcd`x2`
%\footnote{Strictly, there are a few other requirements;
...
%% CHECK: vj -- Check that we permit a class to implement multiple function types.
...
value of type 
\xcdmath"T[y1/x$_1$, $\ldots$, yn/x$_n$]" where \xcdmath"y1,$\ldots$,yn" may be thought of 
as new variables defined as if by:
\begin{xten}
val y1=e1;
...
val yn=en;
...
\begin{itemize}
\item \xcd`3` is of type \xcd`Long{a!=0}` with \xcd`a` replaced by \xcd`3`, 
\viz{}  \xcd`Long{3 != 0}`; 
\item \xcd`4` is of type \xcd`Long{b!=a}` 
...
\xcd`Long{self != a}` with 
 \xcd`a` replaced by \xcd`3` and 
\xcd`b` replaced by \xcd`4`, 
which is to say, \xcd`Long{self!=3}`.
\end{ex}
...
%(FROM #(prod:ClosureBody)#)
         ClosureBody \: Exp & (\ref{prod:ClosureBody}) \\
                     \| ClosureBodyBlock \\
%(FROM #(prod:ClosureBodyBlock)#)
         ClosureBodyBlock \: Annotations\opt \xcd"{" BlockStmts\opt LastExp \xcd"}" & (\ref{prod:ClosureBodyBlock})\\
...
of statements, or a block terminated by an expression to return. Return statements
may be used in the body of the function to return a value (\Sref{ReturnStatement}). 

The type of a function is a function type as described in \Sref{FunctionType}.  In some
cases the 
return type \Xcd{T} of the function can be omittted and defaults to the type of the
body. If a formal \Xcd{xi} does not occur in any
\Xcd{Tj}, \Xcd{c}, \Xcd{T} or \Xcd{e}, the declaration \Xcd{xi:Ti} may
be replaced by just \Xcd{Ti}. \Eg,  \xcd`(Long)=>7` is the integer function returning
7 for all inputs.
...
\xcd`(Long)=>null` is not well-specified; X10 does not know which type of
\xcd`null` is intended.  
%~~exp~~`~~`~~ ~~ ^^^ Functions40
But \xcd`(Long):Rail[Double] => null` is legal.
...
may access the following variables from outer scopes:
\begin{itemize}
...
%%METHOD-SELECTION%% class Span(low:Long, high:Long) {
%%METHOD-SELECTION%%   def this(low:Long, high:Long) {property(low,high);}
%%METHOD-SELECTION%%   def between(n:Long) = low <= n && n <= high;
%%METHOD-SELECTION%%   static def example() {
%%METHOD-SELECTION%%     val digit = new Span(0,9);
%%METHOD-SELECTION%%     val isDigit : (Long) => Boolean = digit.between.(Long);
...
%%METHOD-SELECTION%% %~todo~exp~~`~~`~~ digit:Span~~class Span(low:Long, high:Long) {def this(low:Long, high:Long) {property(low,high);} def between(n:Long) = low <= n && n <= high;} ^^^ Functions90
%%METHOD-SELECTION%% \xcd`digit.between.(Long)` 
...
%%METHOD-SELECTION%% %~todo~exp~~`~~`~~ digit:Span~~class Span(low:Long, high:Long) {def this(low:Long, high:Long) {property(low,high);} def between(n:Long) = low <= n && n <= high;} ^^^ Functions100
%%METHOD-SELECTION%% \xcd`(n:Long) => digit.between(n)`.
...
%%OP-FUN%% Long.<       $\equiv$ (x: Long, y: Long): Boolean => x < y
%%OP-FUN%% \end{xtenmath}
%%OP-FUN%% 
...
no guarantee that \xcd`f(1)==g(1)` will evaluate to true, since there is no
