...
\newcommand{\ureduce}[0]{\stackrel{+}{\rightsquigarrow}}
...
\mynewcommand{\cH}{\mbox{\codesmaller H}}
...
\newcommand{\finishasynct}[2]{\code{[finish | async](}{\tt
    #1})\lb#2\rb}
\newcommand{\clockedfinishasync}[1]{\code{(async | [clocked] finish)}~{\tt #1}}
\newcommand{\finishasync}[1]{\code{[finish values(}\ol{\hp}\code{)| async]}~#1}
\newcommand{\finish}[2]{\code{finish values(}#1\code{)}~#2}
