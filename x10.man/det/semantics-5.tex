...
We present the reductio rule for two schedulers:
...
\Ref{Figure}{reduction} also defines the \emph{advance relation} $\hS \ureduce{c,a} \hS'$
    the is used to advance a statement to its next phase.
The integer $c$ represent how many \code{clocked async} can advance to the next phase in \hS,
    and $a$ represent how many non-nested \code{async} are in \hS.
...
\end{lstlisting}
can be advanced (where $c=3$ and $a=2$) to
\begin{lstlisting}
async { S1 }
clocked async {
  clocked async {}
...
For example, \RULE{(R-Sequential)+} uses $\hS \ureduce{1+,0} \hUnused$,
...
$\hB ::= \blockt{\ol{\hl}}{\hS}$
& Blocks. \\
$\hS ::=  \epsilon ~|~ \hadvance; ~|~ \hp.\hm(\ol{\hp});  ~|~ \valt{\hx}{\he} ~|~ \hp \leftarrow \hp ~|~
    \pclocked~\finishasync{\hB} ~|~ \hS~\hS$ & Statements. \\ %: locations, parameters, field access, invocation, \code{new}
...
\\\\
$\typerule{
    \cH(\hl)=\hClockedAcc(\hr,\ho,\he) \gap \hl\in \pi
}{
  \hl\leftarrow\hl',\cH \preduce \cH[ \hl \mapsto \hClockedAcc(\hr,\ho,\hr(\he,\hl'))]
}$~\RULE{(R-Clocked-W)}
\quad
$\typerule{
  \cH(\hl)=\hClockedAcc(\hr,\ho,\he) \gap \hl\in\pi
}{
  \hval{\hx}{\hl()}{\hS},\cH \preduce \hS[\ho/\hx], \cH \\
}$~\RULE{(R-Clocked-R)}
\\\\
$\typerule{
...
~
$\typerule{
  \cH(\ha)=\hAcc(\hr,\hv)\gap \hw=\hr(\hv,\hp)  \gap\gap \ha\in \pi
}{
  \ha \leftarrow \hp,\cH \preduce \cH[\ha \mapsto \hAcc(\hr,\hw)]
}$~\RULE{(R-Acc-W)}
\\\\
$\typerule{
...
\caption{FX10 Reduction Rules ($\hS,\cH \preduce \hS',\cH' ~|~\cH'$) for the \emph{concurrent} scheduler
    (\RULE{(R-Sequential)+} is not mandatory in the concurrent scheduler; it was added so that the sequential scheduler will progress if an \hasync cannot advance.)
    The \emph{sequential} scheduler is obtained by removing the two \RULE{(\ldots)-}.
    }
