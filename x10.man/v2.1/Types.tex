...
\index{type system}
The types in X10 are as follows.  These are the {\em semantic} types. Other
...
%
%~~neg
the identifier \xcd`Tripe` may be used as a type.
...
non-zero integer. 
%~~stmt~~`~~`~~ ~~class Tripe{}
Similarly, \xcd`var x : Tripe{x != null};` defines a \xcd`Tripe`-valued
...
\noo{Explain the constraint language}
...
As we will see, \xcd`T{c}` is a subtype of \xcd`T{d}` if every value of type
