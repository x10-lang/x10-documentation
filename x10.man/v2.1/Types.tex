...
Classes may be {\em generic}, i.e.{} defined with one or more type
parameters (\Sref{TypeParameters}).

%%REPETITIOUS%% A class with type parameters is {\em generic}.
%%REPETITOUS%% If a generic class $C$ has $k$ type parameters ($k\geq
%%1$), then $k$ types must be supplied
%%REPETITOUS%% to form a type with $C$ as base.  For example, \xcd`Cell[T]` class
%%REPETITOUS%% provides a container capable of holding a value of type \xcd`T`, or
%%REPETITOUS%% being empty.
%%REPETITOUS%% 
%~~gen
%%REPETITOUS%% % package Types.For.Gripes.Of.Wesley.Snipes;
%~~vis
%%REPETITOUS%% \begin{xten}
class Cell[T] {
%%REPETITOUS%%   var empty : Boolean = true;
  var contents : T;
%%REPETITOUS%%   public def this(t:T) { 
     contents = t; empty = false; 
%%REPETITOUS%%   }
  public def putIn(t:T) { 
%%REPETITOUS%%     contents = t; empty = false; 
  }
%%REPETITOUS%%   public def emptyOut() { empty = true; }
  public def isEmpty() = empty;
%%REPETITOUS%%   public def getOut():T {
     if (empty) throw new Exception("Empty!");
%%REPETITOUS%%      return contents ;
  }
%%REPETITOUS%% }
\end{xten}
%%REPETITOUS%% %~~siv
...
%%REPETITOUS%% Examples of types built with \xcd`Cell` as base: \xcd`Cell[String]`,
%%REPETITOUS%% \xcd`Cell[Int]`.
...
Let \xcd`C` be a class. The null value, represented by the literal
\xcd"null", is a value of every type \xcd`C`, where \xcd`C` names a
class. The type whose values are all instances of \xcd`C` but not
...
Classes, interfaces, and structs may have {\em properties}, public
\xcd`val` instance fields that can be used in types.
%  bound on object creation. 
For example, the
...
\index{concrete type}
A {\em generic class} (struct/interface/type definition) 
is a class (struct/interface/type definition) 
declared with $k \geq 1$ type parameters. 
A generic class (struct/interface/type definition) 
can be used to form a type by supplying $k$ types as arguments within
\xcd`[` \ldots \xcd`]`.
%%When instantiated,
...
\XtenCurrVer{} requires generic methods to be static. 
%~~gen
...
\xcd`+` indicates {\em co-variance},  \xcd`-` indicates {\em
 ...
 interface) \xcd`S` specifying that a particlar parameter position
 (say, \xcd`i`) is co-variant means that 
if \xcd`Si <: Ti` then
\xcd`S[S1,\ldots,Sn] <: S[S1,\ldots, Si-1,Ti,Si+1,\ldots Sn]`.
Similarly, saying that position \xcd`i` is is contra-variant means
that 
if \xcd`Si <: Ti` then
\xcd`S[S1,\ldots, Si-1,Ti,Si+1,\ldots Sn] <: S[S1,\ldots,Sn]`. If the
position is invariant, then no such relationship is asserted between
\xcd`Si <: Ti` 
and
\xcd`S[S1,\ldots, Si-1,Ti,Si+1,\ldots Sn]`. The compiler must perform
...
to use variance. (Some classes/structs/interfaces in the standard
...
\paragraph{Use of type definitions in constrctor invocations}
...
%~~vis
\begin{xten}
type Nonnull[T]{T <: Object} = T{self!=null};
...
To illstrate the applicative rather than generative nature of type
definitions, consider that the following code is legal.
%~~gen
% import x10.util.*;
...
is the same as the type \xcd`S{c,d}`.

\subsection{Syntax of constraints}
\index{constraints!permitted}
\index{constraints!syntax}
...
\end{enumerate}
For an expression \xcd`self.p` to be legal in a constraint, it must be
the case that \xcd`p` is a property. However terms \xcd`t.f` may be
...
\xcd`true`. For instance, the asignment that maps 
...
is a subtype of 
\xcd"T[T1,\ldots, Tn]{d}" 
if \xcd"S[S1,\ldots,Sm]" is a subtype of \xcd"T[T1,\ldts,Tn]" and
\xcd"c" entails \xcd"d".
...
The constraint solver assumes all built-in types have an infinite
number of values. Therefore it does not realize that 
\xcd`a != b && a != c && b != c` is not satisfiable.
...
However, X10's constraint sovler is incomplete. There are situations
...
\xcd`Boolean` (since the type \xcd`Boolean` has only two distinct
values).
...
The following types have default values:
\begin{itemize}
