...
%% vj: Hmm.. where did this grammar come from. Lets make sure we use
%% the same productions throughout the text.
%%
%% Bard: I wrote it.  The actual grammar is pretty turgid.
%% e.g., (13) splits Type into FunctionType and the perplexingly-named 
%% ConstrainedType, and (17) splits ConstrainedType into three subcategories,
%% (which might not have explicit constraints despite being ConstrainedType, 
%% and done of which is simply parenthesized (for grouping I guess)).
%% It may make for good parsing, but it's not going to make good exposition.
%% 
%% We *should* use the same nonterminal names though.
%%
%% Here's how I think stuff corresponds when the names are different: 
%%  [ Types? ] --> TypeArguments  
%%  Types? --> TypeArgumentList 
%%  Constraint --> DepParams, only without ExistentialList? 
%%  TypeVar --> ??  (I don't know that the grammar is phrased this way)
%%  Formals --> FormalParameterList
%%  Formal ---> FormalParemeter
%% 
%%  The stuff about void confuses me.  I don't see it in the grammar, 
%%  but X10 accepts:     val f : (Int)=>void  = (Int):void => {};
%%  but void is not a type, so WTF?
\begin{grammar}
...
Classes may be declared to implement multiple interfaces. Semantically, the
...
%TODO - Yoav points out that the example uses ``var'', which is nonvariant.
...
%TODO: Yoav asks ``What are covariant and contravariant values?''
...
\item The fundamental numeric type (\xcd`Int`, \xcd`UInt`,
      \xcd`Long`, \xcd`ULong`, \xcd`Short`, \xcd`UShort`, \xcd`Byte`,
      \xcd`UByte`, \xcd`Float`, \xcd`Double`) all have default value 0.
...
\item Function types have no default value.
...
%%GRAMMAR%% FunctionType \: TypeParameters\opt \xcd"(" Formals\opt \xcd")" Constraint\opt
%%GRAMMAR%% \xcd"=>" Type Throws\opt \\
%%GRAMMAR%% TypeParameters \: \xcd"[" TypeParameter ( \xcd"," TypeParameter
%%GRAMMAR%% )\star \xcd"]" \\
%%GRAMMAR%% TypeParameter \: Identifier \\
%%GRAMMAR%% Formals \: Formal ( \xcd"," Formal )\star \\
%%GRAMMAR%% \end{grammar}
...
useful for everything.\footnote{The behavioral annotation \xcd`property` is
  explained in \Sref{properties}.} \xcd`a.toString()` produces a
