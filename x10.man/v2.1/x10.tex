...
default and are copied on reference across an \Xcd{at} boundary.
 \Xcd{Any} defines only \Xcd{toString}, \Xcd{typeName},
\Xcd{equals} and \Xcd{hashCode}. It no longer defines \Xcd{home},
\Xcd{at}.  The new struct \Xcd{x10.lang.GlobalRef[T]} is the only
mechanism for creating a globally usable reference to an
object. \Xcd{GlobalRef} has a \Xcd{home} property.

The type modifier \Xcd{!} is no longer used. (It is not needed for
user-defined classes. The typedef \Xcd{GlobalRef[T](p:Place)} can
be used for specifying a constraint  on the \Xcd{home} property of a
\Xcd{GlobalRef}.)
