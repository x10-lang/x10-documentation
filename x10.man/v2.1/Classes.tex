...
mutable (\xcd`val`), immutable (\xcd`var`), or accumulator (\Xcd{acc}).
...
\subsection{\Xcd{clocked} Qualifier}

Clocked fields are discussed in \Sref{ClockedFields}.


\subsection{\Xcd{property} Qualifier}

\noo{A field may be marked as a \Xcd{property}.  ... or not?}
...
\noo{I've seen property annotations on fields -- is that still around?}
...
\noo{acc fields must be specified.}
...
\noo{\Xcd{acc} goes on that list too?}
Call-by-reference, \Xcd{ref} parameters, allows passing in variables for a
method to update, as described in \Sref{RefParameters}. 

\subsection{\Xcd{ref} Parameters}
\label{RefParameters}
\index{ref}
\index{parameter!ref}

A \Xcd{ref} parameter allows a method to modify a \Xcd{var} variable that is
available to the caller.  

%~TODO~gen
% package Classes.RefParameters.Primo.Examplo;
% class Example {
%~TODO~vis
\begin{xten}
...
\noo{Make sure this agrees with Yoav's notes}
...
%(safe)\subsubsection{\Xcd{safe} qualifier}
%(safe)\label{SafeAnnotation}
%(safe)\label{SafeMethod}
%(safe)\index{safe!method}
%(safe)
%(safe)A method may be declared \xcd"safe", indicating that it is 
%(safe)\xcd`pinned`, \xcd`nonblocking`, and \xcd`sequential`.
%(safe)Safe methods are the only methods which can be called inside of \xcd`atomic`
%(safe)and \xcd`when` statements.
...
put \Xcd{this} into a data structure before initializing it, and a concurrent
activity could read it before the initialization is finished.  If the 
line marked \Xcd{ILLEGAL} were uncommented, the following program would
exhibit the problem: 
%~~gen
% package Classes.ConstructorBodies.Methane.Ammonia.Atmosphere;
% import x10.util.*;
%~~vis
\begin{xten}
class Escaper {
...
\subsubsection{Initialization of Fields}

All fields of an object must be given a value before they are used.  There are
several ways that this can be accomplished: 
\begin{itemize}
\item A \Xcd{var} field of a type with a default value, if not otherwise
      initialized, 
      will have its type's default value. (\Xcd{val} fields may not use this
      option; they must be given values explicitly, either by an initializer
      or by a constructor.)  For example, \Xcd{(new C()).zero}
      in the example below will be zero, the default value of an \Xcd{Int}.
%~~gen
% package Classes.Initofields.One;
%~~vis
\begin{xten}
class C {
  var zero : Int;
...
\item A field may have an explicit initializer, in which case its value is the
      value of the initializer.  \Xcd{(new D()).one} is one.
%~~gen
% package Classes.Initofields.Two;
%~~vis
\begin{xten}
class D {
  val one = 1;
...
\item A field may be given a value in a constructor, or in a method called
      from a constructor.  \Xcd{(new E()).x} is two; \Xcd{(new E(3)).x} is
      three. 
%~~gen
% package Classes.Initofields.Three;
%~~vis
\begin{xten}
class E {
   var x : Int;
...
\subsubsection{Calling Another Constructor}

A constructor for class \Xcd{C} can call another constructor, either of
\Xcd{C} itself or of \Xcd{C}'s supertype.  The call to the other constructor
must be the first statement of the constructor body.

%~~gen
% package Classical.Constructorical.Calling.Another.One;
%~~vis
\begin{xten}
class C {
  val a : Int;
...
No fields of \Xcd{this} may be read before the optional \Xcd{this}- or
\Xcd{super}-call. For example, having \Xcd{super(this.x);} as the
constructor-call is illegal, since it reads \Xcd{this.x} before the
constructor-call.

\subsubsection{\Xcd{property} Statement}

The \Xcd{property} statement sets the properties of \Xcd{this}; note that
properties may not be assigned to directly.  For example, 
\Xcd{(new C(10)).x == 10}.
%~~gen
% package Classes.Propertystatement.Squid.Stuffing;
%~~vis
\begin{xten}
...
The properties of \Xcd{this} may be read after the \Xcd{property} statement,
but not before it: 
%~~gen
% package Classes.Propertystatement.Squid.Stuffing.Pillows;
%~~vis
\begin{xten}
class C(x:Int, y:Boolean) {
   val z : Int;
...
\subsubsection{Use of \Xcd{this} in a Constructor Body}

After the optional constructor-call and optional \Xcd{property} statement,
\Xcd{this} {\em can} be used, subject to certain restrictions. 
We say that a method is {\em constructor-like} if it is \Xcd{private},
\Xcd{final}, and obeys the following restrictions.

\begin{itemize}
\item \Xcd{this} may not be assigned to anything. This prevents problematic
      cases such as: 
%~~gen
% package Classes.Usethisinactorbodytwo;
%~~vis
\begin{xten}
class Outer {
   var leak : Inner;
...
%
%~~neg
(It also prevents harmless cases, such as \Xcd{val nonleak = this;}

\item \Xcd{this} may only be used as an argument or receiver of a
      constructor-like method call.  The following class definition is legal.
%~~gen
% package Classes.Usethisinactorbodyone; 
%~~vis
\begin{xten}
class C {
...
%
%~~neg
      It would not be legal if \Xcd{private} or \Xcd{final} were omitted, or
      if \Xcd{ctorLike} called \Xcd{x10.io.Console.OUT.println(this +
      "constructed");}.  (The implicit \Xcd{this.toString()} call is a call to
      a non-constructor-like method.)
\end{itemize}

Constructor code may refer to fields of \Xcd{this} only after they have been
definitely assigned.\Sref{DefiniteAssign}  For example: 
%~~gen
%package Classes.ctordefassignuse;
%~~vis
\begin{xten}
class C {
...
fields of \Xcd{this} which have been initialized at the point that the method
was called.  They differ from true constructor bodies in that only 
constructor bodies, not constructor-like methods, may assign to \Xcd{val}
fields. 
%~~gen
% package Classes.Usethisinactorbodythree;
%~~vis
\begin{xten}
class C {
  val m: Int;
...
\noo{I think the following {\em is} still true, but that should be checked.}
\begin{xten}
...
For example: 
%~~gen
%package Classes.And.Type.Conversions;
%~~vis
\begin{xten}
class Uni(n:Int) {
/*
