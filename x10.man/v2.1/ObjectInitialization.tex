...
A method may be annotated with \xcd`@NonEscaping("v1,v2")`.  This
...
\Xcd{@NonEscaping("v1,v2")} methods specify a list of fields of \Xcd{this}, as
a string argument of the annotation: in this example, the fields are \Xcd{v1}
and \Xcd{v2}.  These are the only fields of \Xcd{this} that can be {\em read}
in the body of the method.  So, \Xcd{x = this.v3} would be forbidden---as
would a call to a method which reads \Xcd{v3}, even if that method were
non-escaping. 
...
\xcd`@NonEscaping("v1,v2")`, then every method which overrides \Xcd{m} in any
...
\Xcd{c} is not. \Xcd{cajoled()} is only called after all fields are set, so it
can read anything.   \Xcd{SeeAlso} is able to override \Xcd{aplomb()}, because
...
place of creation. For example, \Xcd{closure} below is acceptable because it
only refers to fields defined at the point it was written.  \Xcd{badClosure}
...
~
  & {\bf Example}
  & {\bf Prop.}
  & {\bf {\tt \small self==this}}\footnote{Top-level {\tt self} only.}
  & {\bf Prop.Meth.}
  & {\bf {\tt this}}
  & {\bf {fields}}
\\\hline
Type of property 
  & (pos) 
  & %?properties
    yes\footnote{The type of the {$i^{th}$} property may only mention
...
Class Invariant
  ...
  & no %? fields may be used
\\\hline
Supertype\footnote{And super-interface}
  & (pos), (pos)
  & yes%?properties
  & yes%? self==this
  & yes%? property methods
...
Static field\footnote{Type and initializer}
  & (pos) (pos) 
  & no %?properties
  & no %? self==this
  & no %? property methods
...
Instance field\footnote{Type and initializer}
  & (pos), (pos) 
  & yes %?properties
  & yes %? self==this
  & yes %? property methods
...
class Example (
   prop : Int,
   proq : Int{prop != proq}, // (pos)
   pror : Int
   )
   {prop != 0} // (pos)
   extends Supertype[Int{self != prop}] // (pos)
   implements SuperInterface[Int{self != prop}] // (pos)
{
   property def propmeth() = (prop == pror); // (pos)
   static staticField 
      : Cell[Int{self != 0}] // (pos)
      = new Cell[Int{self != 0}](1); // (pos)
   var instanceField 
      : Int {self != prop}
      = prop + 1 as Int{self != prop}; 
   def this(
      a : Int{a != 0},
      b : Int{b != a} // (pos)
      )
      {a != b} // (pos)
      : Example{prop == a && proq == b} // (pos)
   {
      super(); // (pos)
      property(a,b,a); // (pos)
   ...
   def someMethod() = prop + staticField + instanceField;  // (pos)
}
\end{xten}
%~~siv
%
