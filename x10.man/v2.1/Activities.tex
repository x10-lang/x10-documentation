...
ForEachStatement \: 
      \xcd"foreach" \xcd"(" Formal \xcd"in" Expression \xcd")"
          Statement 
ForEachStatement \: 
      \xcd"foreach" \xcd"(" Expression \xcd")"
          Statement 
\end{grammar}

\noo{Check the following!}
...
statement (\Sref{ForAllLoop}). \xcd`for(x in C)S` executes \xcd`S` {\em
  sequentially}. \xcd`foreach(x in C)S`
executes \xcd`S` for each iteration of the loop {\em in parallel}.
It is thus equivalent to:
\begin{xten}
foreach (x in C)
  async S
\end{xten}
If the formal is not used in the statement, it may be elided. 
