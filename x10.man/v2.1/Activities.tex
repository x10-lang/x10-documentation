...
\subsubsection{{\tt at} and Definite Assignment}
\label{AtAndDefiniteAssignment}

% TODO I dunno how to justify this, but it's true.

\xcd`at(p)S` is not considered to definitely assign to variables appearing in
\xcd`S`.  For example, the following code requires initialization of \xcd`a`.
without the \xcd`= 0` clause, it would fail to compile, as \xcd`a` is used in
the \xcd`println` without having definitely been assigned.
%~~gen
%package Activities.AtAndDefAssignment;
%class Example{
%def example(){
%~~vis
\begin{xten}
...
the roots.  (They are initialized to {$0+0i$} because \xcd`at` does not
definitely assign to variables.)
...
\xcd"public static def main(a: Array[String](1)):void" method, otherwise an
exception is thrown
...
executed simultaneously (with \xcd`p` bound to the point). 
