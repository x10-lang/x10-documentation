...
An activity executing the statement \xcd"at (q) S" at a place \xcd`p`
...
Since \xcd`at(e) S` is synchronous, the  usual control-flow
...
This mechanism may be overridden for individual 
classes or
structs as described in \xcd`x10.io.CustomSerialization`. 
...
A struct  of type \xcd`x10.lang.GlobalRef[T]` \ref{GlobalRef}
is serialized as a unique global reference to its contained object
(of type \xcd`T`).   Further details may be found in the documentation 
of \xcd`x10.lang.GlobalRef[T]`.
...
language that starts a new activity.)  \xcd`at` and \xcd`async` may be
combined as needed.  Commonly useful variations include: 
\begin{enumerate}
\item \xcd`async at(p) S`, which spawns an activity locally to execute \xcd`S` at
      \xcd`p`; here \xcd`p` is evaluated by the spawned activity;
\item \xcd`at(p) async S`, which evaluates \xcd`p` and then,  at \xcd`p`, spawns an
      activity to execute \xcd`S`;
\item \xcd`async at(p) async S`, which spawns an activity locally to  evaluate
      \xcd`p` and then, at \xcd`p`, spawns a second activity to execute \xcd`S`.
\end{enumerate}


As a consequence of this, \xcd`S` in \xcd`at(p)S` may contain constructs which
only make sense within a single activity.  For example, 
\begin{xten}
...
to place \xcd`p`, before executing \xcd`S` there. Values are copied unless
they are  only reachable through \xcd`GlobalRef`s and \xcd`transient`
fields.    If \xcd`x` is a
variable name referring to some object \xcd`ob` outside of \xcd`S`, then the
same variable name \xcd`x` refers to a deep copy \xcdmath`ob$'$` of \xcd`ob`
...
levels of reference --- except if the reference involves a \xcd`transient`
field, or a field of type \xcd`GlobalRef`.  Structures are preserved as well: if two fields
...
A \xcd`GlobalRef[T]`  \xcd`g` contains a reference to
a value \xcd`o` of type \xcd`T`, in a form which can be transmitted, and a \xcd`Place`
...
{\tt mathProcessor()} returns a \xcd`Place` suitable for calculation.  In practice
...
%%GRAMMAR%% Statement \: AsyncStatement \\
%%GRAMMAR%% AsyncStatement \: \xcd"async"  Statement \\
%%GRAMMAR%% PlaceExpressionSingleList \: \xcd"(" PlaceExpression \xcd")" \\
%%GRAMMAR%% PlaceExpression \: Expression 
%%GRAMMAR%% \end{grammar} 
...
%%TODO: Check against current grammar
\begin{grammar}
...
%%DUPLICATE%% \section{At expressions}
%%DUPLICATE%% 
%%DUPLICATE%% \begin{grammar}
%%DUPLICATE%% Expression \: \xcd"at" \xcd"(" Expression \xcd")" Expression
%%DUPLICATE%% \end{grammar}
%%DUPLICATE%% 
%%DUPLICATE%% An \Xcd{at} expression evaluates an expression synchronously at the
%%DUPLICATE%% given place and returns its value.  For instance a copy of the
%%DUPLICATE%% value pointed to by a \Xcd{GlobalRef} may be obtained using
%%DUPLICATE%% the \Xcd{fetch} method:
%%DUPLICATE%% %~x~gen
%%DUPLICATE%% % package Activities.AtExpressions.Fetching;
%%DUPLICATE%% % class Example[T] {
%%DUPLICATE%% %~x~vis
%%DUPLICATE%% \begin{xten}
%%DUPLICATE%%   def fetch(g:GlobalRef[T]):T = at (g) g();
%%DUPLICATE%% \end{xten}
%%DUPLICATE%% %~x~siv
%%DUPLICATE%% % } 
%%DUPLICATE%% %~x~neg
%%DUPLICATE%% 
%%DUPLICATE%% The expression evaluation may spawn asynchronous activities. The \Xcd{at}
%%DUPLICATE%% expression will return without waiting for those activities to terminate. That
%%DUPLICATE%% is, \Xcd{at} does not have built-in \Xcd{finish} semantics.
...
%%GRAMMAR%% \begin{grammar}
%%GRAMMAR%% Statement \: AtomicStatement \\
%%GRAMMAR%% AtomicStatement \: \xcd"atomic"  Statement \\
%%GRAMMAR%% \end{grammar}
