...
%%WRONG%% Evaluation is performed left to right, wherever possible.  For example, in 
%%WRONG%% %~x~exp~~`~~`~~f:() => ((Int,Int)=>Int), a:()=>Int, b:()=>Int ~~
%%WRONG%% \xcd`f()(a(),b())`, \xcd`f()` is evaluated, then \xcd`a()`, then \xcd`b()`,
%%WRONG%% and then the application.  
...
\begin{bbgrammar}
 Primary    \:      \xcd"this" & (\ref{prod:Primary})\\%<FROM #(prod:Primary)#
...
\begin{bbgrammar}
 Id    \: identifier & (\ref{prod:Id})\\%<FROM #(prod:Id)#
\end{bbgrammar}
...
\begin{bbgrammar}
 FieldAccess    \: Primary \xcd"." Id & (\ref{prod:FieldAccess})\\%<FROM #(prod:FieldAccess)#
    \| \xcd"super" \xcd"." Id\\
...
\begin{bbgrammar}
 MethodInvocation    \: MethodName TypeArguments\opt \xcd"(" ArgumentList\opt \xcd")" & (\ref{prod:MethodInvocation})\\%<FROM #(prod:MethodInvocation)#
 ...
 ArgumentList    \: Exp & (\ref{prod:ArgumentList})\\%<FROM #(prod:ArgumentList)#
...
\begin{bbgrammar}
 Assignment    \: LeftHandSide AssignmentOperator AssignmentExp & (\ref{prod:Assignment})\\%<FROM #(prod:Assignment)#
 ...
 LeftHandSide    \: ExpName & (\ref{prod:LeftHandSide})\\%<FROM #(prod:LeftHandSide)#
    \| FieldAccess\\
 AssignmentOperator    \: \xcd"=" & (\ref{prod:AssignmentOperator})\\%<FROM #(prod:AssignmentOperator)#
    \| \xcd"*="\\
    \| \xcd"/="\\
    \| \xcd"%="\\
    \| \xcd"+="\\
...
\begin{bbgrammar}
 ConditionalExp    \: 
     ConditionalOrExp \xcd"?" Exp \xcd":" ConditionalExp& (\ref{prod:ConditionalExp})\\%<FROM #(prod:ConditionalExp)#
\end{bbgrammar}
...
\item If the operands have numeric types, they are coerced into the larger of
      the two types (see \Sref{WideningConversion}) and then compared for numeric equality.
\end{itemize}
...
\begin{bbgrammar}
 ClassInstCreationExp    \: \xcd"new" TypeName TypeArguments\opt \xcd"(" ArgumentList\opt \xcd")" ClassBody\opt & (\ref{prod:ClassInstCreationExp})\\%<FROM #(prod:ClassInstCreationExp)#
...
\begin{bbgrammar}
 CastExp    \: 
     CastExp \xcd"as" Type & (\ref{prod:CastExp})\\%<FROM #(prod:CastExp)#\\
\end{bbgrammar}
...
\begin{bbgrammar}
 ...
 & (\ref{prod:RelationalExp})\\%<FROM #(prod:RelationalExp)#
\end{bbgrammar}
...
\begin{bbgrammar}
 SubtypbeConstraint    \: Type  \xcd"<:" Type  & (\ref{prod:SubtypeConstraint})\\%<FROM #(prod:SubtypeConstraint)#
    \| Type  \xcd":>" Type \\
\end{bbgrammar}
...
\begin{bbgrammar}
 RelationalExp    \: RelationalExp \xcd"in" ShiftExp& (\ref{prod:RelationalExp})\\%<FROM #(prod:RelationalExp)#\\
...
\begin{bbgrammar}
 Primary    \: \xcd"[" ArgumentList\opt \xcd"]"& (\ref{prod:Primary})\\%<FROM
                                %#(prod:Primary)#
 ClassInstCreationExp    \:\xcd"new" TypeName \xcd"[" Type \xcd"]" \xcd"[" ArgumentList\opt \xcd"]"& (\ref{prod:ClassInstCreationExp})\\%<FROM #(prod:ClassInstCreationExp)#
...
\begin{bbgrammar}
 CastExp    \:  CastExp \xcd"as" Type& (\ref{prod:CastExp})\\%<FROM #(prod:CastExp)#
\end{bbgrammar}
