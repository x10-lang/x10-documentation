...
\section{\Xcd{acc}: Accumulator Variables}
...
%~~vis
\begin{xten}
val sum = Reducer[Int](0, Int.+);
val product = Reducer[Int](1, Int.*);
\end{xten}
%~~siv
%
%~~neg
...
Reduction is only deterministic if:
\begin{enumerate}
...
\item \Xcd{r.zero} is the identity element for \Xcd{r.apply}; that is, 
      ...
      for all \Xcd{a}.
\end{enumerate}
\Xcd{sum} and \Xcd{product} satisfy all these conditions, and give determinate
reductions.  The compiler does not require or check these, though.  
...
x10.io.Console.OUT.println("Sum = " + s + "; Product = " + p);
\end{xten}
%~~siv
%
%~~neg
