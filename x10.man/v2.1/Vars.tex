...
%
%~TODO~neg
The type declaration \Xcd{T} is optional; if specified, it must be the same
type that the reducer \Xcd{r} uses.
...
\Xcd{acc} instead of \Xcd{var} or \Xcd{val}.  Reducers are not specified; each
accumulator comes with its own reducer.  However, the type \Xcd{T} of the
accumulator {\em is} required.

For example, the following method takes a list of numbers, and accumulates
those that are divisible by 2 in \Xcd{evens}, and those that are divisible by
3 in \Xcd{triples}: 
%~TODO~gen
%package Vars.accumulators.parameters.oscillators.convulsitors.proximators;
%import x10.util.*;
%class Whatever {
%~TODO~vis
\begin{xten}
...
%~TODO~gen
% package Vars.Indexed.Accumulators;
%~TODO~vis
\begin{xten}
