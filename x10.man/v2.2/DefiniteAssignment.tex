...
This information is sufficient for the tests X10 makes.  If \xcd`x` can only
be assigned to once in \xcd`S`, it is definitely assigned.  It is an error if
...
\xcd`continue` and \xcd`break` inside of a loop may be ignored conservatively.  
...
might do anything that a method can do.  
\begin{itemize}
...
\item \xcd`m` cannot initialize \xcd`val` fields of \xcd`this`, unless
      permitted to do so by an annotation; see \Sref{somewhere}.
\end{itemize}


\subsubsection{{\tt at} and \xcd`athome`}

\xcd`at(E)S` performs the assignments of \xcd`E`. Within \xcd`S`, only those
...
%~~gen ^^^ DefiniteAssignment3n5q
% package DefiniteAssignment3n5q;
% KNOWNFAIL-at
% class DefAss { def defass() { 
%~~vis
\begin{xten}
var a : Int;
