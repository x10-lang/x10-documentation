...
of type \xcd`T2` --- and in many cases even that is unnecessary.

There are two forms of casts.  {\em Upcasts} happen when \xcd`T1 <: T2`, that
is, when a value is being cast to a more general type.  Upcasts don't require
any runtime computation at all, since, if \xcd`T1 <: T2`, every value of type
\xcd`T1` is automatically one of type \xcd`T2`.   For example, 
\xcd`"A String" as Object` is an upcast: every \xcd`String` is already an
\xcd`Object`, and no work need be done to make it one.
...
It can be called directly with the unambiguous syntax, as in the 
definition of \xcd`p`.   However, one will generally be better off with a
method in this case. 
\end{ex}
