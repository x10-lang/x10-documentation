...
\begin{itemize}
\item An instance method on \xcd`a`, of the form \xcd`def m(B)`, so that the
      invocation is \xcd`a.m(b)`;
\item A static method on \xcd`a`'s class, of the form \xcd`static def m(B)`, 
      so that the invocation is \xcd`A.m(b)`.
\end{itemize}
...
\begin{itemize}
\item A static method on \xcd`A`, of the form \xcd`static def m(B)`, so that
      the invocation is \xcd`A.m(b)`; 
\end{itemize}
...
\begin{itemize}
\item A constructor on \xcd`A`, of the form \xcd`def this(B)`, so that the
      invocation is \xcd`new A(b)`.
\end{itemize}
...
\item An instance operator on \xcd`A`, defined as 
      ...
      so that the invocation is \xcdmath"a.operator $\star$(b)", 
or
\item A right-hand operator on \xcd`B`, defined as
      \xcdmath"operator (a:A) $\star$ this", 
      so that the invocation is \xcdmath"b.operator ()$\star$(b)"; or
\item A static operator on \xcd`A`, defined as
      \xcdmath"operator (a:A) $\star$ (b:B)", 
      so that the invocation is \xcdmath"A.operator $\star$(a,b)"
; or
\item A static operator on \xcd`B`, if \xcd`A` and \xcd`B` are different
      classes, defined as
      \xcdmath"operator (a:A) $\star$ (b:B)", so that the invocation is 
      \xcdmath"B.operator $\star$(a,b)".
\end{itemize}
\noindent
...
\begin{itemize}
\item \xcdmath"(a as B).operator$\star$(b)"
\item \xcdmath"(b as A).operator () $\star$(a)"
\end{itemize}

An application \xcd`a(b)`, for any number of arguments, can only come from an
application definition: 
\begin{itemize}
\item \xcd`operator this(b:B)`, so that the 
invocation is \xcd`a.operator()(b)`
\end{itemize}
...
\item \xcd`operator this(b:B)=(c:C) {...}`, 
      so that the invocation is \xcd`a.operator()=(b,c)`.
\end{itemize}
...
\item an implicit coercion in \xcd`B`:
      ...
      so that the coercion is \xcd`B.operator[B](a)`;
\item or, failing that, an implicit coercion in \xcd`A`:
      \xcd`static operator this():B`,
      so that the coercion is \xcd`A.operator[B](a)`
\end{enumerate}
...
\item An \xcd`as` operator in \xcd`B`: 
      \xcdmath"static operator (a:A) as B", 
      so that the conversion is \xcd`B.operator as[B](a)`
;
\item or, failing that, an implicit coercion in \xcd`B`:
      \xcd`static operator (a:A):B`, so that the conversion is 
      \xcd`B.operator[B](a)`;
\item or, failing that, an implicit coercion in \xcd`A`:
      \xcd`static operator this():B`, so that the conversion is 
      \xcd`A.operator[B](a)`.
\end{enumerate}
