...
with \xcd`STATIC_CHECKS` on.  (X10's type system does not attempt to propagate 
...
\xcd`z`.  
The \xcd`eq` and \xcd`is` property methods 
%~~gen ^^^ Classes100
...
they have formal parameters of different types (in every instantiation of the
type parameters). 

\begin{ex}
The following overloading of \xcd`m` 
%~~gen ^^^ Classes150
...
But, given the following start of a class definition: 
%~~gen ^^^ Classes4d5e
% package Classes4d5e;
%~~vis
\begin{xten}
...
% }
%~~neg
it would be illegal to add any of the following: 
\begin{itemize}
\item 
\xcd`def m(x:Int)=2`
because the generic instantiation \xcd`Two[Int,Any]` would have two 
\xcd`m(Int)` methods, the \xcd`1`; and \xcd`2` definitions.
\item \xcd`def m[X](x:X)=3`, because
then \xcd`Two[Int,Any]` would allow \xcd`m(1)` to mean one of things: either
the \xcd`1` definition above, or the \xcd`3` definition via generic type
parameter inference (\Sref{TypeParamInfer}).  
\item \xcd`def m(x:U)=4`, because the instantiation \xcd`Two[String,String]`
      (or indeed any one in which \xcd`T` and \xcd`U` were the same) would be
      ambiguous, as the \xcd`1` and \xcd`4` definitions would be identical.
\end{itemize}
...
\noo{How about the conversions out of the class?}
...
%%OP%% They may be invoked by the unambiguous syntax: 
...
\noo{Bard - revise Object Initialization to make sure it conforms to the paper}
