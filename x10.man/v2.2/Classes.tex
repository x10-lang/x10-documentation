...
initializer expression for that field will not be evaluated in that
Place.
...
read \Xcd{F} while the first activity is executing the initializer
expression it will suspend until the first activity finishes
evaluating the initializer and stores the resulting value in \Xcd{F}. 
...
result in the program deadlocking.\footnote{The current \Xten{}
  ...
  of \Xten may be able to dectect this and convert such a deadlock into the
  throwing of an CyclicFieldInitializationException.}.
