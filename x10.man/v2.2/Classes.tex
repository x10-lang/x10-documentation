...
\bard{This subsection got moved from Types.  Put it in the right place, OK?}
...
%##(ClassDecl TypeParamsI TypeParamIList Properties PropertyList Property Guard Super Interfaces InterfaceTypeList ClassBody ClassBodyDecls ClassMemberDecl
\begin{bbgrammar}
%(FROM #(prod:ClassDecl)#)
           ClassDecl \: Mods\opt \xcd"class" Id TypeParamsI\opt Properties\opt Guard\opt Super\opt Interfaces\opt ClassBody & (\ref{prod:ClassDecl}) \\
...
%(FROM #(prod:MethodDecl)#)
          MethodDecl \: MethMods \xcd"def" Id TypeParams\opt Formals Guard\opt HasResultType\opt MethodBody & (\ref{prod:MethodDecl}) \\
                     \| BinaryOperatorDecl \\
                     \| PrefixOperatorDecl \\
                     \| ApplyOperatorDecl \\
                     \| SetOperatorDecl \\
...
\begin{bbgrammar}
%(FROM #(prod:CtorDecl)#)
            CtorDecl \: Mods\opt \xcd"def" \xcd"this" TypeParams\opt Formals Guard\opt HasResultType\opt CtorBody & (\ref{prod:CtorDecl}) \\
\end{bbgrammar}
%##)
