...
is: 
\begin{itemize}
\item The \xcd`advance()` {method} may not be called on any clock. (The
      \xcd`Clock.advanceAll();` method is allowed.)
...
Both the \xcd`clocked finish` and \xcd`clocked async` may use the
