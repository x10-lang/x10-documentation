...
\xcd`Rail` rather than making a new \xcd`Pair` class.}  \xcd`Pair[Int]` holds
two integers; 
...
\xcd`x10.regionarray.Array[Int]{self.rank==1 && self.size == n}`
...
It constructs a rail \xcd`a` of \xcd`Int{self==3}`'s -- integers
which 
are statically known to be 3. 
The only number that can be stored into \xcd`a` is \xcd`3`.  
Then it tricks the ocmpiler into 
thinking that it is a rail of \xcd`Int`, without restriction on the
elements, giving it the name \xcd`b` at that type.  
The cast \xcd`aa as Rail[Int]` is a cast to an instance of a generic type,
...
\begin{xten}
  val a = new Rail[Int{self==3}](10, 3);
  // a(0) = 1; would be illegal
  a(0) = 3; // LEGAL
  val aa = a as Any;
  val b = aa as Rail[Int]; // WARNED with -VERBOSE
  b(0) = 1;
...
%~~vis
\begin{xten}
val inc = (r:Rail[Int], i: Long{i != r.size}) => {
...
\begin{xtenmath}
$B_0=\{$Rail[Int] <: Rail[X]$,$ Rail[Boolean] <: Rail[X]$\}$
\end{xtenmath}
Arbitrarily picking the first relation to {\it Strip} first, we get:
\begin{xtenmath}
$B_1=\{$Int = X$,$ Rail[Boolean] <: Rail[X] $\}$
\end{xtenmath}
and then 
\begin{xtenmath}
$B_2=\{$Int = X$,$ Boolean = X $\}$
...
have to be a value of type \xcd`Rail[Int]`, which it is not.  So type
inference fails at this point.  In this case it is correct: there is no way to
give a proper type to this program.\footnote{
In particular, \xcd`X=Any` doesn't work either.  A \xcd`Rail[Int]` is not a
...
\xcd`Rail[Int]`.  However, if the types of the arguments had simply been 
