...
is of type \xcd`Long{self==1}`, and \xcd`true` is of type
...
A local variable expression consists simply of the name of the local variable, a field of \xcd`this`, 
a formal parameter in scope, etc. It evaluates to the value of the local variable. 
...
Promoting a numeric value to a longer numeric type results in either
sign extension or zero extension depending on whether the target type
is signed or unsigned. For example, \xcd`(255 as UByte) as UInt` is
255 while  \xcd`(255 as Byte) as Int` is -1.
...
The right operand, describing a number of bits, must be a Long: 
%~~exp~~`~~`~~ x:Long, y:Long ~~ ^^^Expressions1l4m
\xcd`x << y`.
...
\begin{ex}
%~~exp~~`~~`~~a:Long,b:Long ~~ ^^^ Expressions110
\xcd`a == b ? 1 : 2`
evaluates to \xcd`1` if \xcd`a` and \xcd`b` are the same, and \xcd`2` if they
are different.   As the type of \xcd`1` is \xcd`Long{self==1}` and of \xcd`2`
is \xcd`Long{self==2}`, the type of the conditional expression has the form
\xcd`Long{c}`, where \xcd`self==1` and \xcd`self==2` both imply \xcd`c`.  For
example, it might be \xcd`Long{true}` -- or perhaps it might be a more accurate
type, like \xcd`Long{self != 8}`. Note that this term has no most accurate type
...
One may not test that an \xcd`Int` variable is \xcd`==` to a long like
\xcd`0`: 
%~~gen ^^^ Expressions6q7k
...
A \xcd`Int` like \xcd`i` can never \xcd`==` a \xcd`Long` like \xcd`100`.
Because \xcd`==` does not permit implicit coercions, \xcd`i`
stays a \xcd`Int`.  The loop must be written with a comparison of two
\xcd`Ints`s: 
%~~gen ^^^ Expressions4u6l
...
Because the operation \xcd`<=` is a regular operator, and thus uses coercions in its
arguments, it is legal (although not recommended) to write the loop as  
...
In this formulation, \xcd`i` will be coerced to a \xcd`Long` on each
loop iteration so it can be compared using \xcd`<=` against \xcd`100`.
...
types.   Even though, say, \xcd`0n` and \xcd`0L` represent the same number,
they are different values and of different types, and hence, \xcd`0n != 0L`.  
The expression \xcd`0n == 0L` does not compile.  However, if you hide type
...
%% CHECK vj: Hmm. Why do we ignore constrained type information.
...
exception 
(\xcd`x10.lang.ClassCastException`) if it is not.  
...
\xcd`n as Long{self != w}` succeeds if \xcd`n != w` --- even if \xcd`w` is a value
...
For example, \xcd`1 as Long{self==2}` may fail to compile, because the compiler
knows that \xcd`1`, which has type \xcd`Long{self==1}`, cannot possibly be of
type \xcd`Long{self==2}`. 

If, for some reason, you need to write one of these forbidden casts, cast to
\xcd`Any` first.  \xcd`(1 as Any) as Long{self==2}` always returns false, but
compiles.
...
\begin{ex}
The following class has an explicit conversion from \xcd`Long` to \xcd`Knot`, and
...
Similarly, \xcd`2` (whose innate type is \xcd`Long{self==2}`)
...
%~~exp~~`~~`~~ ^^^ Expressions200
      \xcd`54.321 as Int` is \xcd`54n`, and 
%~~exp~~`~~`~~ ~~ ^^^ Expressions210
      \xcd`-54.321 as Int` is \xcd`-54n`.
...
\item Casting an unsigned integer type to a signed integer type of the same
      ...
      (\eg,  \\
      \xcd`UInt.MAX_VALUE as Int == -1n`.   Casting an unsigned integer type to
...
\begin{ex}
The code for \Xcd{x10.lang.Point} contains a conversion from a
\xcd`Rail`s of longs to \xcd`Point`s of the same length: 
\begin{xten}
...
%~~exp~~`~~`~~x:Long~~ ^^^ Expressions120
For example, \xcd`3 instanceof Long{self==x}` is an overly-complicated way of
saying \xcd`3==x`.



However, it is a static error if \xcd`e` cannot possibly be an instance of
\xcd`C{c}`; the compiler will reject \xcd`1 instanceof Long{self == 2}` because
\xcd`1` can never satisfy \xcd`Long{self == 2}`. Similarly, \Xcd{1 instanceof
...
%~NO~exp~~`~~`~~r:Array[Long](1) ~~
might expect.  For example, \xcd`r instanceof Array[Long{self != 0}]` does
...
%%NOIN%% \begin{xten}
%%NOIN%% class Cont {
%%NOIN%%    operator this in (Long) = true;
...
%~~gen ^^^ Expressions140
% package Expressions.RailCtor.Primo;
% class Example {
% def example() {
%~~vis
\begin{xten}
...
\xcd`Rail[T]`, where \xcd`T` is the computed common supertype (\Sref{LCA})
of the types of of the expressions  \xcdmath"e$_i$". 

\begin{ex}
The type of
\xcd`[0,1,2]` is \Xcd{Rail[Long]}.    
The type of 
\xcd`[0]` is  \xcd`Rail[Long{self==0}]`. 

To make a \xcd`Rail[Long]` containing just a \xcd`0`, use
\xcd`[0 as Long]`.  The \xcd`as Long` masks more detailed type information, such
...
\begin{ex}
Occasionally one does actually need \xcd`Rail[Long{self==0}]`, 
...
%~~gen ^^^ Expressions170
%package Expressions.RailCtor.Details;
%class Eel{}
%class Example{
%def example(){
%~~vis
...
example, if \xcd`i` is an \xcd`Long` variable, \xcd`i++` increments \xcd`i`,
