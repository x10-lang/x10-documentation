...
variables in the surrounding block. For instance, \xcd`(x:Long)=>x*y`
...
\xcd`Long` (the type of all integers representible in 64 bits). It 
...
(\Sref{ConstrainedTypes}).  \xcd`Long{self!=0}` is the type of \xcd`Long`s
...
\xcd`Long{self==1}` is the type of the \xcd`Long`s which are equal to one.  
...
of \xcd`Long{self == 1}` are all elements of \xcd`Long{self != 0}` as well,
because \xcd`self == 1` logically implies \xcd`self != 0`; so 
\xcd`Long{self  == 1} <: Long{self !=0}` by strengthening.  X10 uses both notions
...
A type given by $(1)$ is an identifier \xcd`M`, like \xcd`Point`, \xcd`Long`, or
\xcd`long`.  It refer to a unit -- a class, struct type, or interface, 
(\Sref{ReferenceTypes}).  
...
of strings, \xcd`Long` to the standard struct type of integers, 
and \xcd`Any` to the interface that describes
all X10 values.  
\xcd`long` is an alias for the type \xcd`Long`, for the
...
\xcd`Rail` rather than making a new \xcd`Pair` class.}  \xcd`Pair[Long]` holds
two integers; 
...
\xcd`Pair[Long]`, above, is an example of a use of the generic type \xcd`Pair`. 
...
A type of form {$(5)$}, \xcd`F`, such as \xcd`(x:Long)=>Long`, is a function type.
...
\begin{ex}
\xcd`Array[Long](1)` is the type of one-dimensional arrays of
integers.  It has one type parameter giving the type of element, here
\xcd`Long`.  It has one value parameter giving the number of dimensions, here
\xcd`1`.  
...
the types \xcd`(x:Long)=>Long{self!=x}` and \xcd`(y:Long)=>Long{self!=y}` are
...
%%NOW-GONE%% in the following system.  For example, \xcd`Array[Long](1)` is the type of
%%NOW-GONE%% one-dimensional integer-valued arrays; it is an abbreviation for
%%NOW-GONE%% \xcd`Array[Long]{rank==1}`.\\
%%NOW-GONE%% 
...
%%NOW-GONE%%  For example,
%%NOW-GONE%% \xcd`Array[Long]` is the type of arrays of integers. 
%%NOW-GONE%% \xcd`HashMap[String,Long]` is the type of hash maps from strings to
%%NOW-GONE%% integers.
%%NOW-GONE%% 
...
%%NOW-GONE%% For example, \xcd`var n : Long{self != 0};` guarantees that \xcd`n` is always a
...
%%NOW-GONE%% \xcd`LinkedList[Long]` is a linked list of integers.
...
%%NOW-GONE%% \xcd`(x:Long) => Long{self != x}` 
...
%%NOW-GONE%% An example of such a function is \xcd`(x:Long) => x+1`.
%%NOW-GONE%% \end{ex}
%%NOW-GONE%% 
...
%%NOW-GONE%% %~~vis
%%NOW-GONE%% \begin{xten}
%%NOW-GONE%%   val plus1 : (x:Long) => Long{self != x} = 
%%NOW-GONE%%      (x:Long) => (x+1) as Long{self != x}; 
%%NOW-GONE%% \end{xten}
%%NOW-GONE%% %~~siv
%%NOW-GONE%% %}
%%NOW-GONE%% %~~neg
...
%%NOW-GONE%% \xcd`(n:Long)=>Long{self!=n}`
%%NOW-GONE%% describes integer-valued functions that have no fixed point
%%NOW-GONE%% 
%%NOW-GONE%% 
%%NOW-GONE%% For example, \\
%%NOW-GONE%% \xcd`(a:Long, b:Long{self!=a})=>Long{self!=a, self!=b}` 
%%NOW-GONE%% and \\
%%NOW-GONE%% \xcd`(c:Long, d:Long{self!=c})=>Long{self!=c, self!=d}` \\
...
in a rail is a property of the rail, and an X10 program can specify that
two rails have the same number of elements.
...
\xcd`Stack[Long]` is a stack of longs, and 
...
the type parameters explicitly (\eg, \xcd`first[Long](z)`).
...
used to create an object \xcd`a` of type \xcd`Cont[Long]`, and an object
\xcd`b` which implements \xcd`Inte[Long]`.  The two types may be given aliases
...
constraints.  The type of \xcd`n`-element \xcd`Array[Long](1)`s  is 
%~~type~~`~~`~~n:Long ~~ ^^^ Types250
\xcd`x10.regionarray.Array[Long]{self.rank==1 && self.size == n}`
...
\xcd`String`. Similarly, \xcd`A` and \xcd`Long` are equivalent.
%~~gen ^^^ Types280
...
Basic types, like \xcd`Long` and \xcd`List[String]`, provide useful
...
type \xcd`T` which is being constrained.  For example, \xcd`Long{self == 4}` is
the type of \xcd`Long`s which are equal to 4 -- the best possible description
...
\begin{itemize}
%~~type~~`~~`~~ ~~ ^^^ Types290
\item \xcd`Long{self != 0}` is the type of non-zero \xcd`Long`s. 
\item \xcd`Long{self == 0}` is the type of \xcd`Long`s which are zero.  
\item \xcd`Long{self != 0, self != 1}` is the type of \xcd`Long`s which are
      neither zero nor one.
\item \xcd`Long{self == 0, self == 1}` is the type of \xcd`Long`s which are both
...
\item Suppose that \xcd`Matrix` is a matrix class with  properties \xcd`rows`
      and \xcd`cols`.  
%~~type~~`~~`~~ ~~class Matrix(rows:Long,cols:Long){} ^^^ Types300
...
% class Matrix(rows:Long, cols:Long){
...
%%FAILY-jira-XTENLANG-2779%%      val a : Array[Long{self!=a.rank}] = null;
...
\begin{itemize}
\item \xcd`Long{self == 3} <: Long{self != 14}`.  The only value of
      \xcd`Long{self ==3}` is $3$.  All integers but $14$ are members of
      \xcd`Long{self != 14}`, and in particular $3$ is.  
...
%%TYPES-CONSTR-EXP%% 
%%TYPES-CONSTR-EXP%% For instance, \xcd"v: Long{v == 0}" is shorthand for \xcd"v: Long{self == 0}".
%%TYPES-CONSTR-EXP%% 
%%TYPES-CONSTR-EXP%% 
...
values of incompatible types are not allowed.  If \xcd`i:Long`, then \xcd`i==0`
...
%%NOW-GONE%% %~~vis
%%NOW-GONE%% \begin{xten}
%%NOW-GONE%% class Listlike(x:Long) {
%%NOW-GONE%%   val kid : Listlike{self.x == this.x};
%%NOW-GONE%%   def this(x:Long, kid:Listlike) { 
%%NOW-GONE%%      property(x); 
...
It constructs a rail \xcd`a` of \xcd`Long{self==3}`'s -- integers
which 
are statically known to be 3. 
The only number that can be stored into \xcd`a` is \xcd`3`.  
Then it tricks the ocmpiler into 
thinking that it is a rail of \xcd`Long`, without restriction on the
elements, giving it the name \xcd`b` at that type.  
The cast \xcd`aa as Rail[Long]` is a cast to an instance of a generic type,
which is the problem. 

But, itc an store any \xcd`Long` into the elements of 
\xcd`b`, thereby violating 
the invariant that all the elements of the rail are 3.  
...
type of \xcd`i` to avoid one of the many longs which are not possible subscripts:  
...
\item The fundamental numeric types (\xcd`Int`, \xcd`UInt`,
  ...
  \xcd`Short`, \xcd`UShort`, \xcd`Byte`,
   \xcd`UByte`, 
      \xcd`Float`, \xcd`Double`) all have default value \xcd`0`.
...
\begin{ex}
\xcd`var x: Long{x != 4}` has default value 0, which is allowed
because \xcd`0 != 4` satisfies the constraint on \xcd`x`. 
\xcd`var y : Long{y==4}` has no default value, because \xcd`0` does not satisfy \xcd`y==4`.
The fact that \xcd`Long{y==4}` has precisely one value, \viz{} 4, doesn't
matter; the only candidate for its default value, as for any subtype of
\xcd`Long`, is 0. \xcd`y` must be initialized before it is used.
\end{ex}
...
class hierarchy): every child is a person.  Similarly, \xcd`Long{self != 0}`
is a subtype of \xcd`Long` -- every non-zero integer is an integer.  
...
on the context.    \xcd`Long{self != 0} <: Long` is always
true.
...
%~~exp~~`~~`~~test:Boolean ~~ ^^^ Types420
      \xcd`test ? 0 : 1`;
%~~exp~~`~~`~~ ~~ ^^^ Types430
\item Rail construction, like \xcd`[0, "non-zero"]` and 
%~~exp~~`~~`~~ ~~ ^^^ Types440
      \xcd`[0,1]`;
...
The
best type of \xcd`0` 
is \xcd`Long{self==0}`, and the best type of 1 is \xcd`Long{self==1}`.
Certainly {$E$} could be given the type \xcd`Long`, or even \xcd`Any`, and that
would describe all possible results.  However, we actually know more.
\xcd`Long{self != 2}` is a better description of the type of {$E$}---certainly
the result of {$E$} can never be \xcd`2`.   \xcd`Long{self != 2, self != 3}` is
...
\xcd`Long{self == 0 || self == 1} `, which would be nearly perfect.  But 
...
\item If \xcd`T == U`, then \xcd`V == T == U`.  For example, 
      ...
      \xcd`Long{c}` for some constraint \xcd`c`---perhaps simply 
      picking \xcd`Long{true}`, \viz, \xcd`Long`. 
...
is, a supertype of all types.\footnote{Java, for one, suffers a number of
  inconveniences because some built-in types like \xcd`long` and \xcd`char`
...
(\xcd`return;` rather than \xcd`return e;`) is said to be a \xcd`void` method or closure.
...
syntactic place of a type in a few contexts. A \xcd`void` method can be specified by
...
the formally correct but rather more awkward ``return type of a method that is not a 
\xcd`void` method''.   Despite this informal usage, \xcd`void` is not a type and cannot be used as 
the value of a type parameter.  For
example, given 
%~~gen ^^^ Types530
...
%~~neg
\noindent
The call \xcd`eval[void](f)` does {\em not} typecheck. There is no way in X10 to
write a generic function which works with both functions which return a value
and functions which do not.  In such cases it may be convenient to define a type 
\xcd`Unit` thus:
%~~gen ^^^ Types531
% package Types.unittype;
% class EEEEVi1 {
%~~vis
\begin{xten}
  struct Unit{}
...
%~~neg
\noindent
Functions whose return type is \xcd`Unit` may simply return the expression \xcd`Unit()` which
evaluates to the unique value of type \xcd`Unit`. (By definition of equality of structurs
\xcd`Unit()==Unit()`.)
...
Failure of the compiler to infer unique types for omitted type arguments
is a compile-time error. For instance, given the method
definition \xcd`def m[T](){...}`, an invocation \xcd`m()` is considered a
compile-time error.  The compiler has no idea what \xcd`T` the programmer intends.
...
\xcd`choose[Long](1, 2)` picks an integer, 
...
\xcd`Rail[String]{length==3}`.  When doing type inference, 
the compiler is able to infer that \xcd`T` should be instantiated to \xcd`String`:
%~~gen ^^^ Types3d5j
% package Types3d5j;
% class Example {
%~~vis
...
\end{xtenmath}
Then, the compiler is seeking a
set {$B$}  of type 
bindings 
\begin{xtenmath}
...
\begin{xtenmath}
$B_0=\{$Rail[Long] <: Rail[X]$,$ Rail[Boolean] <: Rail[X]$\}$
\end{xtenmath}
Arbitrarily picking the first relation to {\it Strip} first, we get:
\begin{xtenmath}
$B_1=\{$Long = X$,$ Rail[Boolean] <: Rail[X] $\}$
\end{xtenmath}
and then 
\begin{xtenmath}
$B_2=\{$Long = X$,$ Boolean = X $\}$
\end{xtenmath}
...
have to be a value of type \xcd`Rail[Long]`, which it is not.  So type
inference fails at this point.  In this case it is correct: there is no way to
give a proper type to this program.\footnote{
In particular, \xcd`X=Any` doesn't work either.  A \xcd`Rail[Long]` is not a
\xcd`Rail[Any]` --- and it must not be, for you can put a boolean value into
a \xcd`Rail[Any]`, but you cannot put a boolean value into an
\xcd`Rail[Long]`.  However, if the types of the arguments had simply been 
\xcd`X` rather than \xcd`Rail[X]`, then type inference would correctly infer
\xcd`X=Any`.
}
\end{ex}
...
\begin{ex}
The following code creates a one-element rail (\Sref{sect:RailCtors})
containing \xcd`x`.  
...
has type \xcd`Extra`, and thus \xcd`[x as Extra]` is a \xcd`Rail[Extra]` as
desired. 
