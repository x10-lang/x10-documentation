FX10 is an effects system for X10 intended to complement Featherweight Java
...
executed in a heap that satisfies the constraint $\phi$, statement $S$ will on
termination result in a heap that satisfies $\psi$.  Since we do not
model \code{null}, our formalization can be particularly simple:
...
% todo
\Ref{Section}{Typing} defines the flow-sensitive typing rules
    ($\phi~\hS~\psi$), while
    \Ref{Section}{Reduction} gives the reduction rules for statements and expressions ($\hS,\hH \reducesto \hS',\hH' ~|~\hH'$ and~$\he,\hH \reduce \hl,\hH'$)
    and our soundness theorem.
...
the other over E-configurations. The rules defining these relations are 
standard. The only minor novelty is in how \async{} is defined. The
