...
%    The design should prevent initialization-related errors.
  ...
  \item[Type safe]
    ...
    Adding non-null types to Java~\cite{Fahndrich:2003:DCN:949305.949332,Fahndrich:2007:EOI:1297027.1297052,XinQi:2009}
        ...
        Java's too relaxed initialization rules.
\end{description}
...
\Ref{Section}{Global-references} describes how global references are used in distributed code
...
Note how \code{fib(n-2)} is calculated asynchronously at the next place,
    while simultaneously calculating \code{fib(n-1)} in the current place.
When both calculations finish, we sum their result into the \emph{final} field \code{fib}.
...
Details of those can be found in X10's language specification.
...
Java has definite-assignment rules (using an intra-procedural data-flow analysis)
    ...
    (see \Ref{Section}{Read-write-rules}).
...
Leaking \this to another place before it is fully initialized
    might also cause bugs in the custom serialization code (see \Ref{Section}{Multiple-Places}).
