...
This hardhat design in X10 (version 2.1) has the above desirable properties,
...
The \emph{contributions of this paper} are:
    ...
    (ii)~implementation inside the X10 open-source compiler and
        converting the entire X10 code-base (+400K lines of code) %according to Igor
        to conform to the hardhat principles,
    (iii)~FX10 formalism which is the first to present a flow-sensitive effect system with concurrency constructs
    and a soundness theorem stating that one can never read an uninitialized field in a well-typed program.
