...
class A {
  val f:Int;
  def this() { //err: f was not assigned
...
In contrast, X10 performs an \emph{inter}-procedural (fixed-point) data-flow analysis in all \emph{non-escaping methods} to calculate
...
The details are explained in this section with examples by comparison with Java's definite-assignment rules;
    the full analysis is described in X10's language specification.


Java allows \emph{writing} to a final field only when it is definitely-\emph{unassigned},
    and it allows \emph{reading} from a final field only when it is definitely-\emph{assigned}.
X10 adds another

However, Java does not check that a method that reads a final field is called only after that field was assigned.


The inter-procedural analysis for final fields does not need a fixed-point calculation
    because
...
class B {
  var i:Int{self!=0}, j:Int{self!=0}, flag:Boolean;
  def this() {
    flag = false;
    // assigned={flag}
    finish {
...
\caption{Read-Write order for fields examples.
    We infer for each method three sets:
        ...
        (ii)~fields it writes,
        (iii)~fields it asynchronously writes.
    In the example, we omitted empty sets.
    \myrule{\arabic{Rule11}}{A field can be read only after it was definitely assigned}
    }
